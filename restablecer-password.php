<?php
/**
 * Restablecimiento de contrasena del Equipo Clinico mediante token.
 */
...
try {
    $db = getDB();
    $reset = obtenerResetUsuarioValido($db, $token);
} catch (PDOException $e) {
    error_log('Restablecer password staff (validacion) error: ' . $e->getMessage());
    $reset = null;
...
if ($tokenValido && ($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($nueva === '' || $confirmacion === '') {
        ...
        try {
            restablecerPasswordUsuario($db, (int) $reset['reset_id'], (int) $reset['usuario_id'], $nueva);
            $_SESSION['alerta_login'] = 'Tu contrasena fue actualizada. Ya puedes iniciar sesion.';
            header('Location: ' . BASE_URL . '/login.php');
            exit;
        } catch (PDOException $e) {
            error_log('Restablecer password staff (update) error: ' . $e->getMessage());
...
<head>
    ...
    <title>Restablecer contrasena - DentiSoft</title>
...
<body>
    <main class="portal-card">
        <?php if (!$tokenValido): ?>
            ...
            <div class="warn">
                Por seguridad los enlaces vencen a los <?= USUARIO_RESET_TOKEN_TTL_MINUTES ?> minutos y solo pueden usarse una vez.
                Solicita uno nuevo para continuar.
            </div>
            <a class="back" href="<?= BASE_URL ?>/recuperar-password.php">&larr; Solicitar un nuevo enlace</a>
        <?php else: ?>
            <h1>Define tu nueva contrasena</h1>
            <p>Ingresa una contrasena nueva para tu cuenta del equipo clinico.</p>
